...
currentSite = window.location.host.split('.')[1] === 'squig' ? window.location.host.split('.')[0] : window.location.host.split('.')[1] === 'hangout' ? 'crinacle' : 'superreview';
    ...
    $.getJSON(squigsitesJson, function(squigSites) {
        ...
        squigSites.forEach(function(site) {
            let name = site.name,
                ...
                baseUrl = username === 'crinacle' ? 'https://graph.hangout.audio/' : 'https://' + username + '.squig.link',
                url = username === 'superreview' ? 'https://squig.link' : 'https://' + username + '.squig.link',
                phoneBookJson = username === 'superreview' ? 'https://squig.link/data/phone_book.json' : 'https://'+ username +'.squig.link/data/phone_book.json',
                dbs = site.dbs;
            ...
            dbs.forEach(function(db) {
                let siteContainer = document.createElement('section'),
                    ...
                    let folder = db.folder,
                        ...
                        phoneBookJson = baseUrl + folder + 'data/phone_book.json';
